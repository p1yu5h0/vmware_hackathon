...
import { getRegionWiseAssetQuantityAPI } from "../../api";
...
const RegionQuantity = () => {
  const [regionWiseAssetQuantity, setRegionWiseQuantity] = useState({
    labels: [],
    values: [],
    min : 0,
    max : 0,
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result7 = await getRegionWiseAssetQuantityAPI();
      setRegionWiseQuantity({
        labels: Object.keys(
          result7.data.data["Region Asset Quantity"]["Ordered Qty"]
        )
          .map((key) => decodedData["Region"][key])
          .slice(1),
        values: Object.values(
          result7.data.data["Region Asset Quantity"]["Ordered Qty"]
        ).slice(1),
        min : Math.ceil(result7.data.data["Min"]["Ordered Qty"]),
        max : Math.ceil(result7.data.data['Max']["Ordered Qty"]),
  ...
  return (
    <div>
      <div className='grid grid-cols-3 gap-8'>
        <Key title='Maximum Expenditure Prediction' value={regionWiseAssetQuantity.max} icon={<BsGraphUp size={25}/>}/>
        <Key title='Minimum Expenditure Prediction' value={regionWiseAssetQuantity.min} icon={<BsGraphDown size={25}/>}/>
        <Key title='Average Yearly Expenditure' value={regionWiseAssetQuantity.avg} icon={<GoGraph size={25}/>}/>
      </div>
      ...
      <Graph
          ...
          labels={regionWiseAssetQuantity.labels}
          datasets={[
            {
              label: "Region wise asset quantity",
              data: regionWiseAssetQuantity.values,

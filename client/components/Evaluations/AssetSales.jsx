...
import { getAssetWiseSalesAPI } from '../../api';
...
const AssetSales = () => {
  const [assetWiseSales, setAssetWiseSales] = useState({
    labels : [],
    values : [],
    max : 0,
    min : 0,
  ...
  useEffect(()=>{
    const fetchData = async()=>{
      const result3 = await getAssetWiseSalesAPI();
      setAssetWiseSales({
        labels : Object.keys(result3.data.data['Asset Wise Sales']['Total Sales']).map(key=>decodedData.Category[key]),
        values : Object.values(result3.data.data['Asset Wise Sales']['Total Sales']),
        max : parseInt( result3.data.data['Max']['Total Sales']),
        min : parseInt(result3.data.data['Min']['Total Sales']),
        avg : parseInt(result3.data.data['Mean']['Total Sales']),
      });
      setLoading(false);
    }
    fetchData();
  ...
  return (
    <div>
      <div className='grid grid-cols-3 gap-8'>
        <Key title='Maximum Expenditure Prediction' value={assetWiseSales.max} icon={<BsGraphUp size={25}/>}/>
        <Key title='Minimum Expenditure Prediction' value={assetWiseSales.min} icon={<BsGraphDown size={25}/>}/>
        <Key title='Average Yearly Expenditure' value={assetWiseSales.avg} icon={<GoGraph size={25}/>}/>
      </div>
      ...
      <Graph type={GRAPH_TYPES.BAR_CHART} labels={assetWiseSales.labels} datasets={[{
          label : 'Assets wise Sale',
          data : assetWiseSales.values,
